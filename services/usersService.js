...
const sendVerificationEmail = async (user) => {
    ...
    const verifyLink = `${process.env.FRONTEND_URL || 'http://localhost:7885'}/verify-email/${token}`;
...
const sendResetPasswordEmail = async (email) => {
    ...
    const resetLink = `${process.env.FRONTEND_URL || 'http://localhost:7885'}/reset-password?token=${token}`;
    // Send email via queue
    await addSendEmailJob({
        to: user.emailID,
