const model = require('../model/presaleSectionGroupsModel');
...
const createGroupService = async (groupData) => {
  try {
    ...
    console.error('presaleSectionGroupsService.createGroupService error', error);
    return { success: false, message: 'Internal server error', error: error.message };
  }
};

const addProductsService = async (groupID, presaleProductIDs) => {
  try {
    ...
    if (!Array.isArray(presaleProductIDs) || presaleProductIDs.length === 0) {
      return { success: false, message: 'presaleProductIDs must be a non-empty array' };
    }

    const normalized = presaleProductIDs.map(p => (p === null || p === undefined ? '' : String(p).trim())).filter(p => p.length > 0);
    if (normalized.length === 0) return { success: false, message: 'No valid presaleProductIDs provided' };
    ...
    return { success: true, message: 'Presale products added to group' };
  } catch (error) {
    console.error('presaleSectionGroupsService.addProductsService error', error);
    return { success: false, message: 'Internal server error', error: error.message };
  }
};

const replaceProductsService = async (groupID, presaleProductIDs) => {
  try {
    const gid = parseInt(groupID, 10);
    if (Number.isNaN(gid)) return { success: false, message: 'Invalid groupID' };
    if (!Array.isArray(presaleProductIDs)) return { success: false, message: 'presaleProductIDs must be an array' };
    const normalized = presaleProductIDs.map(p => (p === null || p === undefined ? '' : String(p).trim())).filter(p => p.length > 0);
    ...
    console.error('presaleSectionGroupsService.replaceProductsService error', error);
    return { success: false, message: 'Internal server error', error: error.message };
  }
};
...
const updateGroupService = async (groupID, data) => {
  try {
    ...
    console.error('presaleSectionGroupsService.updateGroupService error', error);
    return { success: false, message: 'Internal server error', error: error.message };
  }
};
...
const deleteGroupService = async (groupID) => {
  try {
    ...
    console.error('presaleSectionGroupsService.deleteGroupService error', error);
    return { success: false, message: 'Internal server error', error: error.message };
  }
};
...
const getGroupByIDService = async (groupID) => {
  try {
    ...
    console.error('presaleSectionGroupsService.getGroupByIDService error', error);
    return { success: false, message: 'Internal server error', error: error.message };
  }
};
...
const listGroupsService = async (query = {}) => {
  try {
    ...
    console.error('presaleSectionGroupsService.listGroupsService error', error);
    return { success: false, message: 'Internal server error', error: error.message };
  }
};
...
const getGroupProductsService = async (groupID) => {
  try {
    ...
    console.error('presaleSectionGroupsService.getGroupProductsService error', error);
    return { success: false, message: 'Internal server error', error: error.message };
  }
};
